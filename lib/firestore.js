...
// ---- applications ----
function apps(uid) { return tenants.doc(uid).collection('applications'); }

async function createApplication(uid, data) {
  const ref = apps(uid).doc();
  await ref.set({
    ...data,
    status: 'in_progress',
    createdAt: FieldValue.serverTimestamp(),
  ...
  // private index so the public answer/submit/resume endpoints can resolve
  // appId -> uid WITHOUT the applicant ever seeing the tenant uid.
  await db.collection('applicationIndex').doc(ref.id).set({ uid });
  return ref.id;
}
async function getApplicationUid(appId) {
  const s = await db.collection('applicationIndex').doc(String(appId)).get();
  return s.exists ? s.get('uid') : null;
}
async function getApplication(uid, appId) {
  const snap = await apps(uid).doc(String(appId)).get();
  return snap.exists ? { appId: snap.id, ...snap.data() } : null;
}
async function updateApplication(uid, appId, patch) {
  await apps(uid).doc(String(appId)).set({ ...patch, updatedAtMs: Date.now() }, { merge: true });
}
async function deleteApplication(uid, appId) {
  await apps(uid).doc(String(appId)).delete();
}
// one active application per identity: look up an in-progress/submitted doc by
// the identity hash (no index needed - small per-tenant collection).
async function findApplicationByIdentity(uid, identityHash) {
  const snap = await apps(uid).where('identityHash', '==', identityHash).limit(5).get();
  const live = snap.docs.map((d) => ({ appId: d.id, ...d.data() }))
    .filter((a) => a.status === 'in_progress' || a.status === 'submitted' || a.status === 'reinterview');
  live.sort((a, b) => (b.createdAtMs || 0) - (a.createdAtMs || 0));
  return live[0] || null;
}
async function listApplications(uid, limit = 100) {
  const snap = await apps(uid).limit(500).get();
  const rows = snap.docs.map((d) => ({ appId: d.id, ...d.data() }));
  rows.sort((a, b) => (b.createdAtMs || 0) - (a.createdAtMs || 0));
  return rows.slice(0, limit);
}
// ---- Concierge ----
async function getConciergeConfig(uid) {
  const { DEFAULTS, runtimeView } = require('./concierge/config');
  const snap = await tenants.doc(uid).collection('concierge').doc('config').get();
  const data = snap.exists ? snap.data() : {};
  const merged = { ...DEFAULTS, ...data };
  merged.runtimeView = runtimeView(merged);
  return merged;
}
async function setConciergeConfig(uid, validatedPatch) {
  await tenants.doc(uid).collection('concierge').doc('config')
    .set({ ...validatedPatch, updatedAt: FieldValue.serverTimestamp() }, { merge: true });
}
// per-player onboarding session
function csessions(uid) { return tenants.doc(uid).collection('conciergeSessions'); }
async function getConciergeSession(uid, playerId) {
  const s = await csessions(uid).doc(String(playerId)).get();
  return s.exists ? { playerId: s.id, ...s.data() } : null;
}
async function setConciergeSession(uid, playerId, data) {
  await csessions(uid).doc(String(playerId)).set({ ...data, updatedAtMs: Date.now() }, { merge: true });
}
async function listConciergeSessions(uid, limit = 50) {
  const snap = await csessions(uid).limit(300).get();
  const rows = snap.docs.map((d) => ({ playerId: d.id, ...d.data() }));
  rows.sort((a, b) => (b.arrivedAtMs || 0) - (a.arrivedAtMs || 0));
  return rows.slice(0, limit);
}
// funnel events (append-only; aggregated by analytics)
async function recordConciergeEvent(uid, event) {
  await tenants.doc(uid).collection('conciergeEvents').add({ ...event, atMs: Date.now(), createdAt: FieldValue.serverTimestamp() });
}
async function listConciergeEvents(uid, limit = 2000) {
  const snap = await tenants.doc(uid).collection('conciergeEvents').limit(limit).get();
  return snap.docs.map((d) => ({ id: d.id, ...d.data() }));
}
// GDPR-style purge: drop sessions + events (owner action / retention window)
async function purgeConciergeData(uid) {
  for (const col of ['conciergeSessions', 'conciergeEvents']) {
    const snap = await tenants.doc(uid).collection(col).limit(500).get();
    const batch = db.batch();
...
// per-IP+slug apply throttle (hashed IP, TTL-ready), like allowSignup
async function allowApply(ipHash, limitPerHour) {
  const ref = db.collection('rl_apply').doc(ipHash);
  return db.runTransaction(async (tx) => {
    const s = await tx.get(ref);
    const now = Date.now();
    const windowStart = s.exists ? Number(s.get('windowStart') || 0) : 0;
    const count = s.exists ? Number(s.get('count') || 0) : 0;
    if (now - windowStart >= 3_600_000) { tx.set(ref, { windowStart: now, count: 1, expireAt: new Date(now + 7_200_000) }); return true; }
    if (count >= limitPerHour) return false;
    tx.update(ref, { count: count + 1 });
    return true;
  });
