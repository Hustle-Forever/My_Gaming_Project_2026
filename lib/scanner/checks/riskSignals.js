...
const PATTERNS = [
  {
    checkId: 'risk-shell', severity: 'critical', label: 'shell-execution call',
  ...
  {
    checkId: 'risk-http', severity: 'high', label: 'outbound HTTP request',
  ...
  {
    checkId: 'risk-credentials', severity: 'high', label: 'hardcoded secret',
  ...
  {
    checkId: 'risk-obfuscation', severity: 'medium', label: 'obfuscated blob',
          ...
          for (let i = 0; i < lines.length; i++) {
            if (pat.re.test(lines[i])) {
              out.push({
                ...
                // LOCATION + category only - never the verbatim source line
                // (would leak raw code / secrets into stored findings).
                evidence: [{ resource: res.name, file: file.path, line: i + 1, detail: pat.label }],
              });
