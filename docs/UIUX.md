...
- **Whitelist Officer** (dashboard + public `apply.html`) — Queue/Setup tabs + a 4-cell stats strip; queue rows with a score badge (lime/amber/red), identity, status pill, flag count; a detail view with a conic score gauge, per-criterion cards quoting evidence (violet start-border, italic), a scrollable transcript (officer/applicant bubbles), and Approve/Reject/Re-interview + note + Delete. The public interview page reuses the console's chat bubbles, voice mic, and orb; EN/AR RTL throughout, 380px-first.
- **The Concierge** (dashboard) — a safety line (shield, lime) stating notify-only up front; enable toggle, a tone segmented control, and a check-in-delay slider with a live `Xm Ys` read-out; then a live analytics read: an **onboarding funnel** as labelled lime→violet gradient bars (arrived→greeted→answered→reached→checked-in), three **retention** stat tiles, an **arrivals-over-time** mini bar chart (violet→lime columns, per-day labels), and ranked **question-theme** bars. Fresh tenants get a dashed empty state instead of zeroed charts. Charts re-render on language switch; EN/AR RTL, 380px-first. See [CONCIERGE.md](CONCIERGE.md).
