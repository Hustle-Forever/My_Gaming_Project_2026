...
- **Requests mic permission BEFORE `recognition.start()`.** The machine's `requestMic` preflight (`getUserMedia`) runs first, so the user gets a clean permission prompt and — on failure — a precise `DOMException` instead of an opaque `start()` throw. `recognition.start()` only runs after the mic is granted; the granted stream also drives the listening orb.
- **Errors are actionable, with the underlying `DOMException` surfaced.** Permission denial (`NotAllowedError`/`SecurityError`, whether from the preflight or a `start()` throw) → *"Microphone blocked — click the icon in the address bar… and allow access"*; no device (`NotFoundError`/`NotReadableError`) → its own message; anything else → *"Couldn't start the microphone…"* — each with the `[Name: message]` appended so a stuck user (or the debug panel) sees exactly what the browser said. Permission/device errors are **not** retried (only `InvalidStateError` is).
