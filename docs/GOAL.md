...
- ✅ Platform hardened and proven: **`npm test` 88/88** (auth + mandatory email verification, pay-gate cycle, key custody, interpretation incl. rogue-provider whitelist proof, queue, envelope, rate limiting, **+ the read-only Server Scanner**) + `npm run smoke:emulator` 15/15.
